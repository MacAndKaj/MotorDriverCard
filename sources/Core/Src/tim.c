...
/* USER CODE BEGIN 0 */
#define TIM_DMA_ONE_WORD 1
#define TIM_DMA_ONE_HALFWORD 1
/* USER CODE END 0 */
...
void MX_TIM15_Init(void)
{
  ...
  htim15.Init.Prescaler = SERVO_TIM15_PRESCALER;
  htim15.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim15.Init.Period = SERVO_TIM15_PERIOD;
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef* tim_pwmHandle)
{
  ...
  /* USER CODE END TIM15_MspInit 0 */
    ...
    hdma_tim15_ch1_up_trig_com.Init.MemInc = DMA_MINC_DISABLE;
    ...
    /* Several peripheral DMA handle pointers point to the same DMA handle.
     Be aware that there is only one channel to perform all the requested DMAs. */
    __HAL_LINKDMA(tim_pwmHandle,hdma[TIM_DMA_ID_CC1],hdma_tim15_ch1_up_trig_com);
    __HAL_LINKDMA(tim_pwmHandle,hdma[TIM_DMA_ID_UPDATE],hdma_tim15_ch1_up_trig_com);
    __HAL_LINKDMA(tim_pwmHandle,hdma[TIM_DMA_ID_TRIGGER],hdma_tim15_ch1_up_trig_com);
    __HAL_LINKDMA(tim_pwmHandle,hdma[TIM_DMA_ID_COMMUTATION],hdma_tim15_ch1_up_trig_com);
...
void HAL_TIM_MspPostInit(TIM_HandleTypeDef* timHandle)
{
  ...
  /* USER CODE END TIM15_MspPostInit 0 */
    ...
    PB14     ------> TIM15_CH1
    */
    GPIO_InitStruct.Pin = Scan_Servo_PWM_Pin;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = GPIO_AF1_TIM15;
    HAL_GPIO_Init(Scan_Servo_PWM_GPIO_Port, &GPIO_InitStruct);
...
{
    HAL_StatusTypeDef state = HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_3, pwm_source, TIM_DMA_ONE_WORD);
    if (state != HAL_OK)
    {
        Error_Handler();
    }
...
{
    HAL_StatusTypeDef state = HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_1, pwm_source, TIM_DMA_ONE_WORD);
    if (state != HAL_OK)
    {
        Error_Handler();
    }
...
{
    HAL_StatusTypeDef state = HAL_TIM_PWM_Start_DMA(&htim15, TIM_CHANNEL_1, (uint32_t*)pwm_source, TIM_DMA_ONE_HALFWORD);
    if (state != HAL_OK)
    {
        Error_Handler();
    }
...
void start_tim_encoder(TIM_HandleTypeDef *htim)
{
    HAL_StatusTypeDef state = HAL_TIM_Encoder_Start(htim, TIM_CHANNEL_ALL);
    if (state != HAL_OK)
    {
        Error_Handler();
    }
...
{
    start_tim_encoder(&htim8);
}

void start_tim_4_encoder(void)
{
    start_tim_encoder(&htim4);
}

int16_t get_tim_encoder_count(TIM_HandleTypeDef *htim)
{
...
{
    return get_tim_encoder_count(&htim8);
}

int16_t get_tim_4_encoder_count(void)
{
    return get_tim_encoder_count(&htim4);
}
