...
{
    HAL_SPI_StateTypeDef state = HAL_SPI_GetState(&hspi3);
    if (state != HAL_SPI_STATE_READY)
    {
        return state;
    }

    uint32_t err = HAL_SPI_GetError(&hspi3);
    if (err != HAL_SPI_ERROR_NONE)
    {
        return -(int)err;
    }

    HAL_StatusTypeDef status = HAL_SPI_Receive_DMA(&hspi3, buffer, buffer_size);
    if (status != HAL_OK)
    {
        return status;
    }
...
int start_spi_2_dma_transfer(uint8_t* data, uint16_t data_size)
{
    ...
    HAL_StatusTypeDef status = HAL_SPI_Transmit_DMA(&hspi3, tx_buffer, data_size);
    if (status != HAL_OK)
    {
        return status;
    }
...
int start_spi2_dma_transfer_and_reception(uint8_t* tx_data, uint8_t* rx_buffer, uint16_t data_size)
{
    ...
    HAL_StatusTypeDef status = HAL_SPI_TransmitReceive_DMA(&hspi3, tx_data, rx_buffer, data_size);
    if (status != HAL_OK)
    {
        return status;
    }
